...
const KycDetailsModal = ({ formData = {}, onClose }) => {
        ...
        <td className="p-2 border text-center">
          ...
          {file ? (
            <>
              {typeof file === "string" ? (
                <>
                  {/* Show only "View" if not Approved */}
                  {formData.KycStatus !== "Approved" ? (
                    ...
                    // If KYC is approved, show only View
                    <button
                      onClick={() => handleViewImage(file)}
                ...
                <>
                  ...
                  {/* Only show Change if KYC is not Approved */}
                  {formData.KycStatus !== "Approved" && (
                    <button
                      onClick={() => handleUploadClick(key)}
                      className="text-blue-600 hover:underline"
            ...
            // Only show Select File if not Approved
            formData.KycStatus !== "Approved" && (
              <button
                onClick={() => handleUploadClick(key)}
  ...
  return (
    <div className="fixed inset-0 z-50 flex   justify-center items-center bg-black bg-opacity-40">
      <div className="bg-white w-full max-h-[90vh] overflow-y-scroll hide-scrollbar max-w-4xl rounded shadow-lg overflow-hidden">
            ...
            <tbody>
              <tr>
                ...
                <td className="p-2 border">
                  <div>
                    <input
                      type="text"
                      className={`w-full border p-1 ${error ? "border-red-500" : ""
                        }`}
                      value={aadhaarNumber}
                      onChange={handleChange}
                      placeholder="Enter Aadhaar number"
                      maxLength={12}
                      disabled={uploading !== "" || formData.KycStatus === "Approved"}
                    />
                    {error && (
              ...
              <tr>
                ...
                <td className="p-2 border">
                  <input
                    type="text"
                    className={`w-full border p-1 ${panError ? "border-red-500" : ""
                      }`}
                    value={panNumber}
