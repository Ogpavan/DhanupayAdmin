...
const UserTypeAndRoleManager = () => {
  ...
  // Fetch token and UserId only once when component mounts
  useEffect(() => {
    const Token = Cookies.get("token");
    const UserId = Cookies.get("UserId");
  ...
  // Fetch user types whenever the token or userId is available
  useEffect(() => {
    const fetchUserTypes = async () => {
    ...
    fetchUserTypes();
  }, [token, userId]);  // Fetch only when token or userId changes

  // Fetch roles when selectedUserType or token changes
  useEffect(() => {
    const fetchRoles = async () => {
      if (!token || !userId) return;  // Ensure token and userId are available
      try {
  ...
  const createRole = async () => {
    ...
    try {
      ...
      console.log("newRole.description:", newRole.description);
