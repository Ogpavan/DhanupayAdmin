...
const DistributorSection = () => {
 ...
 console.log(allUsers)
  ...
  // Fetch wallet balances when Distributer is selected
  const fetchWalletBalances = async (DistributerId) => {
    setLoadingBalances(true);
    try {
      const response = await axios.post(
        "https://gateway.dhanushop.com/api/WalletMaster/WalletMasterList",
        { UserId: DistributerId },
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
...
//   }, [walletBalances]);
  ...
  const handleTransfer = async () => {
    // console.log(selectedDistributer, selectedWallet, amount);
    if (!selectedDistributer || !selectedWallet || !amount) {
      Swal.fire({
        icon: "warning",
...
if (transactionType=="Debit") {
  ...
  const headers = [
    { label: "Distributer Name", key: "Distributer_name" },
  ...
  const data = [
    {
      Distributer_name: "Distributer 1",
          ...
          <div className="p-6">
            <form>
              <div className="mb-6">
               ...
               {selectedDistributer.length > 0 && showDropdown && (
  <ul className="absolute z-10 w-full bg-white border border-gray-200 rounded-md shadow-lg max-h-48 overflow-y-auto">
    {users
      .filter((user) =>
        user.AgentId?.toString().toLowerCase().includes(selectedDistributer.toLowerCase())
      )
      .slice(0, 10)
      .map((user) => (
        <li
          ...
          onClick={() => {
            setSelectedDistributer(user.NewUserID);
        ...
        <div className="bg-white shadow-md rounded-lg">
          <div className="border-b p-4">
            <h2 className="text-lg font-semibold flex items-center text-indigo-700">
              ...
              Distributer Transaction History
            </h2>
          </div>
          <div className="p-6">
