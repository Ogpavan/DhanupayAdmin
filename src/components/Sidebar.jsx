...
import { NavLink } from "react-router-dom";
import { House, Users, Wallet, Gear, UserCircleGear, Notepad, CaretDown, CaretUp } from "phosphor-react";
import Cookies from "js-cookie";
...
export default function Sidebar() {
  ...
  const usersidebarLinks = [
    { to: "/user", label: "Dashboard", icon: <House size={24} weight="fill" /> },
    {
      ...
      nested: [
        { to: "/user/services/rechargess", label: "Recharges" },
         { to: "/user/services/recharges", label: "bills payment" },
        { to: "/user/services/aeps", label: "AEPS" },
        { to: "/user/services/OthersServices", label: "Aadhaar Pay " },
         { to: "/user/services/OthersServicess", label: "DMT" },
                  ...
                  <div className="pl-10 flex flex-col space-y-1">
                    {link.nested.map((nestedLink, nestedIndex) => (
                      <NavLink
                        ...
                        to={nestedLink.to}
                        className={({ isActive }) =>
                          `text-sm py-1 hover:text-indigo-700 ${
